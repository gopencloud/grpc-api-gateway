...
func TestFieldMaskFromRequestBody(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			msg:      &testpb.NonStandardMessageWithJSONNames{},
...
`,
		...
		},
		{
			name:     "protobuf-any",
			msg:      &testpb.ABitOfEverything{},
			input:    `{"anytype":{"@type": "xx.xx/testpb.NestedOuter", "one":{"two":{"three":{"a":true, "b":false}}}}}`,
			expected: newFieldMask("anytype"), //going deeper makes no sense
		},
		{
			name:     "repeated-protobuf-any",
			msg:      &testpb.ABitOfEverything{},
			input:    `{"repeated_anytype":[{"@type": "xx.xx/testpb.NestedOuter", "one":{"two":{"three":{"a":true, "b":false}}}}]}`,
...
func TestFieldMaskRepeatedFieldsLast(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			actual, err := partialfieldmask.FieldMaskFromRequestBodyJSON(bytes.NewReader([]byte(tc.input)), &testpb.ABitOfEverything{})
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestFieldMaskErrors(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := partialfieldmask.FieldMaskFromRequestBodyJSON(bytes.NewReader([]byte(tc.input)), &testpb.ABitOfEverything{})
